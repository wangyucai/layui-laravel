layui.config({base: '/layadmin/modul/common/'}).use(['table', 'dialog', 'his'], function(){
    ...
    table.render({
        ...
        ,cols: [[ //表头
            ...
            ,{field: 'xxhjs_code', title: '信息化技术代码'}
            ,{field: 'xxhjs_name', title: '信息化技术名称'}
    ...
    table.on('tool(infortechtab)', function(obj){
        var data = obj.data;      //获得当前行数据
        var layEvent = obj.event; //获得 lay-event 对应的值（也可以是表头的 event 参数对应的值）
        var tr = obj.tr;          //获得当前行 tr 的DOM对象

        if (layEvent == 'edit') {
            dialog.open('编辑信息化技术代码', '/admin/infortech/'+data.id+'/edit');
        } else if (layEvent == 'del') {
            dialog.confirm('确认删除改信息化技术代码么', function () {
    ...
    // 排序
    table.on('sort(infortechtab)', function (obj) {
        var cond = $('.search_input').val();
        flushTable(cond, obj);
    });
    ...
    $('.add_btn').click(function () {
        dialog.open('添加信息化技术代码', '/admin/infortech/create');
    });
});