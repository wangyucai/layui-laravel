layui.config({base: '/layadmin/modul/common/'}).use(['table','form','dialog', 'his'], function(){
    ...
    table.on('tool(myfixedassettab)', function(obj){
        var data = obj.data;      //获得当前行数据
        var layEvent = obj.event; //获得 lay-event 对应的值（也可以是表头的 event 参数对应的值）
        var tr = obj.tr;          //获得当前行 tr 的DOM对象

        if (layEvent == 'look') {
            dialog.open('查看我的固定资产', '/admin/myfixedasset/look/'+data.id);
            ...
            dialog.confirm('确认归还么', function () {
                var loadIndex = dialog.load('归还中，请稍候');
                var newStatus = (data.if_back == 1) ? 0 : 1;
                his.ajax({
                    url: '/admin/myfixedasset/back'
                    ,type: 'post'
                    ,data: {id: data.id, if_back: newStatus}
                    ,complete: function () {
                        dialog.close(loadIndex);
                    }
                    ,error: function (msg) {
    ...
    function flushTable (cond,lqrq_start, lqrq_end, sortObj) {
        var query = {
            where: {
                cond: cond,
        ...
        table.reload('myfixedassets', query);
    }
    ...
    $('.search_btn').click(function () {
        ...
        flushTable(cond,lqrq_start, lqrq_end);
    });
    ...
    table.on('sort(myfixedassettab)', function (obj) {
        ...
        var lqrq_start = $('#lqrq_start').val();
        var lqrq_end = $('#lqrq_end').val();
        flushTable(cond,lqrq_start, lqrq_end, obj);
    });

    // 添加装备资产
    $('.add_btn').click(function () {
        dialog.open('添加我的固定资产', '/admin/myfixedasset/create');
    });
});