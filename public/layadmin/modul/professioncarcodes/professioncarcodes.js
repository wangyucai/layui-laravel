layui.config({base: '/layadmin/modul/common/'}).use(['table', 'dialog', 'his'], function(){
    ...
    table.render({
        elem: '#professioncarcodes'
        ,url: '/admin/professioncarcodes' //数据接口
        ,method: 'get'
        ,page: true //开启分页
        ,limit: 10
        ,limits: [10, 20]
        ...
        ,cols: [[ //表头
            ...
            ,{field: 'car_code', title: '职业证书代码'}
            ,{field: 'car_name', title: '职业证书名称'}
    ...
    table.on('tool(professioncarcodetab)', function(obj){
        var data = obj.data;      //获得当前行数据
        var layEvent = obj.event; //获得 lay-event 对应的值（也可以是表头的 event 参数对应的值）
        var tr = obj.tr;          //获得当前行 tr 的DOM对象

        if (layEvent == 'edit') {
            dialog.open('编辑职业资格证书代码', '/admin/professioncarcode/'+data.id+'/edit');
        } else if (layEvent == 'del') {
            dialog.confirm('确认删除该职业资格证书代码么', function () {
    ...
    function flushTable (cond, sortObj) {
        ...
        table.reload('professioncarcodes', query);
    }
    ...
    // 排序
    table.on('sort(professioncarcodetab)', function (obj) {
        var cond = $('.search_input').val();
        flushTable(cond, obj);
    });
    ...
    $('.add_btn').click(function () {
        dialog.open('添加职业资格证书代码', '/admin/professioncarcode/create');
    });
});