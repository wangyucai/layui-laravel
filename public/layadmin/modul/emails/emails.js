layui.config({base: '/layadmin/modul/common/'}).use(['table', 'dialog', 'his'], function(){
    ...
    table.render({
        ...
        ,cols: [[ //表头
            ...
            ,{field: 'email_content', title: '邮件内容'}
            ,{field: 'username', title: '邮件发送者'}
    ...
    table.on('tool(emailtab)', function(obj){
        var data = obj.data;      //获得当前行数据
        var layEvent = obj.event; //获得 lay-event 对应的值（也可以是表头的 event 参数对应的值）
        var tr = obj.tr;          //获得当前行 tr 的DOM对象

        if (layEvent == 'edit') {
            dialog.open('编辑邮件', '/admin/email/'+data.id+'/edit');
        } else if (layEvent == 'del') {
            dialog.confirm('确认删除该邮件么', function () {
    ...
    // 排序
    table.on('sort(emailtab)', function (obj) {
        var cond = $('.search_input').val();
        flushTable(cond, obj);
    });
    ...
    $('.add_btn').click(function () {
        dialog.open('发送邮件', '/admin/email/create');
    });
});