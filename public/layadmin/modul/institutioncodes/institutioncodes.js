layui.config({base: '/layadmin/modul/common/'}).use(['table', 'dialog', 'his'], function(){
    ...
    table.render({
        elem: '#institutioncodes'
        ,url: '/admin/institutioncodes' //数据接口
        ,method: 'get'
        ,page: true //开启分页
        ,limit: 10
        ,limits: [10, 20]
        ...
        ,cols: [[ //表头
            ...
            ,{field: 'jdjg_code', title: '司法鉴定机构代码'}
            ,{field: 'jdjg_dwdm', title: '司法鉴定机构所属单位代码'}
            ,{field: 'jdjg_name', title: '司法鉴定机构名称'}
    ...
    table.on('tool(institutioncodetab)', function(obj){
        var data = obj.data;      //获得当前行数据
        var layEvent = obj.event; //获得 lay-event 对应的值（也可以是表头的 event 参数对应的值）
        var tr = obj.tr;          //获得当前行 tr 的DOM对象

        if (layEvent == 'edit') {
            dialog.open('编辑管理员', '/admin/institutioncode/'+data.id+'/edit');
    ...
    function flushTable (cond, sortObj) {
        ...
        table.reload('institutioncodes', query);
    }
    ...
    // 排序
    table.on('sort(institutioncodetab)', function (obj) {
        var cond = $('.search_input').val();
        flushTable(cond, obj);
    });
    ...
    $('.add_btn').click(function () {
        dialog.open('添加司法鉴定机构代码', '/admin/institutioncode/create');
    });
});