layui.config({base: '/layadmin/modul/common/'}).use(['table', 'dialog', 'his'], function(){
    ...
    table.render({
        ...
        ,cols: [[ //表头
            ...
            ,{field: 'code', title: '内设机构代码'}
            ,{field: 'code_name', title: '内设机构名称'}
    ...
    table.on('tool(mechanismcodetab)', function(obj){
        var data = obj.data;      //获得当前行数据
        var layEvent = obj.event; //获得 lay-event 对应的值（也可以是表头的 event 参数对应的值）
        var tr = obj.tr;          //获得当前行 tr 的DOM对象

        if (layEvent == 'edit') {
            dialog.open('编辑内设机构代码', '/admin/mechanismcode/'+data.id+'/edit');
        } else if (layEvent == 'del') {
            dialog.confirm('确认删除改内设机构代码么', function () {
    ...
    function flushTable (cond, sortObj) {
        ...
        table.reload('mechanismcodes', query);
    }
    ...
    // 排序
    table.on('sort(mechanismcodetab)', function (obj) {
        var cond = $('.search_input').val();
        flushTable(cond, obj);
    });
    ...
    $('.add_btn').click(function () {
        dialog.open('添加内设机构代码', '/admin/mechanismcode/create');
    });
});