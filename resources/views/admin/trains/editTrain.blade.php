	...
	<form class="layui-form layui-form-pane" style="width: 60%">
		<div class="layui-form-item">
			...
			<div class="layui-input-block">
				<input type="text" class="layui-input" name="px_title" lay-verify="required" placeholder="请输入培训项目名称" value="{{ $trains['px_title'] }}">
			</div>
		</div>
		<div class="layui-form-item">
			<label class="layui-form-label">培训地点</label>
			<div class="layui-input-block">
				<input type="text" class="layui-input" name="px_place" lay-verify="required" placeholder="请输入培训地点" value="{{ $trains['px_place'] }}">
			</div>
		</div>
		<div class="layui-form-item">
			<div class="layui-inline">
				...
				<div class="layui-input-inline">
					<input type="text" id="px_start_time" class="layui-input px_start_time" lay-verify="required|date" name="px_start_time" placeholder="请选择培训开始日期" value="{{ date('Y-m-d', $trains['px_start_time']) }}">
				</div>
			</div>
			<div class="layui-inline">
				<label class="layui-form-label">培训结束日期</label>
				<div class="layui-input-inline">
					<input type="text" id="px_end_time" class="layui-input px_end_time" lay-verify="required|date" name="px_end_time" placeholder="请选择培训结束日期" value="{{ date('Y-m-d', $trains['px_end_time']) }}">
				</div>
			</div>
		</div>
		<div class="layui-form-item">
		    ...
		    <div class="layui-input-block">
		        <select name="px_fx" lay-filter="px_fx" lay-verify="required">
	                ...
	                @foreach($jd_data as $val)
                    	<option @if(($fx->pxfx_code.','.$val->jdywfw_code) == $trains['px_fx'])) selected="selected" @endif value="{{$fx->pxfx_code.','.$val->jdywfw_code}}">{{$val->jdywfw_name}}</option>
                    @endforeach
               		</optgroup>
	                @elseif($fx->pxfx_code == '04')
	                <optgroup  label="{{ $fx->pxfx_name }}">
	                @foreach($xinxi_data as $val2)
                        <option @if(($fx->pxfx_code.','.$val2->xxhjs_code) == $trains['px_fx'])) selected="selected" @endif value="{{$fx->pxfx_code.','.$val2->xxhjs_code}}">{{$val2->xxhjs_name}}</option>
                    @endforeach
                    </optgroup>
	                @else
	                <option @if($fx->pxfx_code == $trains['px_fx'])) selected="selected" @endif  value="{{$fx->pxfx_code}}" >{{$fx->pxfx_name}}</option>
	                @endif
	                @endforeach
		        </select>
		    </div>
		...
		<div class="layui-form-item">
			...
			<div class="layui-input-block">
				<input type="text" class="layui-input" name="px_renshu" lay-verify="required" placeholder="请输入培训人数" value="{{ $trains['px_renshu'] }}">
			</div>
		</div>
		<div class="layui-form-item">
			<label class="layui-form-label">主办单位</label>
			<div class="layui-input-block">
				<input id="zbdw_id" type="text" class="layui-input" name="zbdw_id" lay-verify="required" placeholder="请输入主办单位" value="{{ $trains['name'] }}">
				<select lay-filter="lishi">		        
					<option value="">主办单位历史记录</option>
			        @foreach($zhuban as $v)
                    <option @if($v->name == $trains['name'])) selected="selected" @endif value="{{ $v->name }}" >{{ $v->name }}</option>
                    @endforeach
		       </select>
			</div>
			</div>
		...
		<input type="hidden" name="px_id" value="{{ $trains['id'] }}">
		<div class="layui-form-item">
			<div class="layui-input-block">
